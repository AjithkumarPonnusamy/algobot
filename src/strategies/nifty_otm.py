    ...
    def execute_strategy_logic(self):
        if self.atm_ce_1_ohlc is not None:
            ...
            ce_high = self.atm_ce_1_ohlc['high']
             # Entry condition
            if ce_low < self.otm_bep and ce_close > self.otm_bep and self.position["CE"] is None:
            ...
            # Target checks
                ...
                if ce_close < self.otm_bep and ce_high > self.otm_bep and self.position["CE"] is not None:
                    sell_qty = entry_data["quantity"]
                    if sell_qty > 0:
        ...
        if self.atm_pe_1_ohlc is not None:
            ...
            # Entry condition
            if pe_close > self.otm_bep and pe_low < self.otm_bep and self.position["PE"] is None:
            ...
            # Exit condition - falls below BEP
            elif pe_close < self.otm_bep and self.position["PE"] is not None:
            ...
            # Target checks
                ...
                if pe_close >= entry_data["target2"]:
                 ...
                 # Exit condition - falls below BEP
                if pe_close < self.otm_bep and pe_high > self.otm_bep and self.position["PE"] is not None:
                    sell_qty = entry_data["quantity"]
                    if sell_qty > 0:

...
class NiftyATMStrategyFloor():
    def __init__(self, feed, quantity=150, underlying_symbol="13"):
        ...
        #Stoploss Checks
        self.last_stoploss_candle_time = None
        self.stoploss_count = 0
        self.max_stoploss_allowed = 3
        self.stoploss_limit_reached = False
    ...
    def execute_strategy_logic(self):
        ...
        if self.atm_ce_1_ohlc is not None:
            ...
            ce_ltp = self.atm_ce_strike_ltp
             # Entry condition
            if (ce_low <= self.atm_bep and ce_close > self.atm_bep
             and self.position["CE"] is None
             and not self.trade_completed
             and self.stoploss_count < self.max_stoploss_allowed  # ⬅️ check here
             and (self.last_stoploss_candle_time is None or self.atm_ce_1_ohlc["time"] > self.last_stoploss_candle_time)):
                     ...
                     if entry_data["quantity"] > 0:
                        ...
                        if self.atm_ce_1_ohlc and ce_final:
                            ...
                            if ce_close < self.atm_bep and ce_high > self.atm_bep:
                                ...
                                self.last_stoploss_candle_time = current_candle_time  # ⬅️ block re-entry this candle  
                                self.stoploss_count += 1  # ⬅️ increment SL counter
        ...
        if self.atm_pe_1_ohlc is not None:
            ...
            # Entry condition
            if (pe_close > self.atm_bep and pe_low <= self.atm_bep
                and self.position["PE"] is None
                and not self.trade_completed
                and self.stoploss_count < self.max_stoploss_allowed  # ⬅️ check here
                and (self.last_stoploss_candle_time is None or self.atm_pe_1_ohlc["time"] > self.last_stoploss_candle_time)):
            ...
            # Target checks
                ...
                if not entry_data["hit_t1"] and pe_ltp >= entry_data["target1"]:
                    sell_qty = entry_data["quantity"] / 2
                    if sell_qty > 0:
                        print(f"[TARGET] CE hit Target 1 at {pe_ltp} (+15) → Sell {sell_qty}")
                        self.place_order(
                            price=pe_ltp,
                            order_type="Limit",
                            side="Sell",
               ...
               # Second Target -> Exit
                if pe_ltp >= entry_data["target2"]:
                    sell_qty = entry_data["quantity"]
                    if sell_qty > 0:
                        print(f"[TARGET] CE hit Target 2 at {pe_ltp} (+30) → Exit {sell_qty}")
                        self.place_order(
                            price=pe_ltp,
                            order_type="Limit",
                            side="Sell",
    ...
    def stoploss_check(self):
            if self.stoploss_count >= self.max_stoploss_allowed and not self.stoploss_limit_reached:
                msg = f"🛑 Max Stoploss limit ({self.max_stoploss_allowed}) reached. No more trades today."
                print(msg)
                send_telegram_message(msg)
                self.stoploss_limit_reached = True  # ⬅️ prevent repeated alerts
